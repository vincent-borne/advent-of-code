...
bool is_outside(Pipe pipes[200][200], Pipe* pipe, int x_limit) {
    int count_pipe_x = 0;

    for (int x = pipe->x + 1; x < x_limit; x++) {
        Pipe* one = &pipes[pipe->y][x];
        ...
        for (int xx = x + 1; xx < x_limit; xx++) {
            Pipe* two = &pipes[pipe->y][xx];
            ...
            if (one->type == L && two->type == J) {
                count_pipe_x += 2;
                break;
            } else if (one->type == L && two->type == S) {
                count_pipe_x++;
                break;
            } else if (one->type == F && two->type == S) {
                count_pipe_x += 2;
                break;
            } else if (one->type == F && two->type == J) {
                count_pipe_x++;
                break;
            }
        }
...
int main(void) {
...

    {
        ...
        current_pipe->is_main_loop = true;
        Pipe* queue[200];
    ...
    for (int y = 0; y < pipes_y_size; y++) {
        for (int x = 0; x < pipes_x_size; x++) {
            if (pipes[y][x].is_main_loop) {
                continue;
            }

            if (!is_outside(pipes, &pipes[y][x], pipes_x_size)) {
                ++result;
            }
        }
    }
