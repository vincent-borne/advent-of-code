...
int main(void) {
    ...
    for (int i = 0; i < reports_size; i++) {
        ...
        for (int j = report->generated_line_size - 1; j > 0; j--) {
            ...
            int* p_line_size = &report->generated_size[j - 1];

            int* line = report->generated[j];
            int* line_size = &report->generated_size[j];

            p_line[*p_line_size] = p_line[*p_line_size - 1] + line[*line_size - 1];
            (*p_line_size)++;
        }

        report->original[report->original_size] = report->original[report->original_size - 1] + report->generated[0][report->generated_size[0] - 1];
        result += report->original[report->original_size];
        report->original_size++;
    }
