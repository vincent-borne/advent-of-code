...
int main(void) {
    ...
    while ((read = getline(&line, &len, f)) != -1) {
        ...
        lines[lines_size] = malloc(sizeof(char) * strlen(split_path[0]) + 1);
        strcpy(lines[lines_size], split_path[0]);
        lines_nb_size[lines_size] = strlen(split_path[0]);
        ...
        for (int i = 0; i < split_a_size; i++) {
            arrangments[lines_size][i] = atoi(split_a[i]);
        }

        arrangments_size[lines_size] = split_a_size;
        lines_size++;
    }
