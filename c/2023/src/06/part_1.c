...
int main(void) {
    ...
    while ((read = getline(&line, &len, f)) != -1) {
        ...
        if (line_count == 0) {
            ...
            for (int i = 1; i < split_times_size; i++) {
                printf("%s, %d\n", split_times[i], atoi(split_times[i]));
                time[time_size] = atoi(split_times[i]);
                time_size++;
            }
            ...
            for (int i = 1; i < split_distances_size; i++) {
                distance[distance_size] = atoi(split_distances[i]);
                distance_size++;
            }
        }
    ...
    for (int i = 0; i < time_size; i++) {
        for (int j = 0; j < time[i]; j++) {
            int speed = j;
            int time_left = time[i] - j;
            int new_distance = speed * time_left;
